...
export default function ResponseIntegration({ definition, response, request }) {
  ...
  useEffect(() => {
    ...
    listBranches().then(items => {
      ...
      if (current) setError('Não foi possível carregar as coordenações estaduais. Feche e abra esta integração para tentar novamente.');
  ...
  return <details className={styles.card} onToggle={event => setOpened(event.currentTarget.open)}><summary>Usar resposta em: {target.label}</summary>
    ...
    {loadingBranches && <p role="status">Carregando coordenações estaduais…</p>}
    {error && <p role="alert">{error}</p>}
    <label>Coordenação estadual<select value={branchId} onChange={event => setBranchId(event.target.value)} disabled={loadingBranches}><option value="">{loadingBranches ? 'Carregando…' : 'Selecione'}</option>{branches.map(branch => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>
