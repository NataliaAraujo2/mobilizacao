...
function RecipientImport({ onAdd, run }) {
  ...
  return <section className={styles.card}>
    ...
    <label>Buscar em<select value={source} onChange={e => { setSource(e.target.value); setSearch(''); setResult(null); }}><option value="branches">Responsáveis das coordenações estaduais</option><option value="volunteers">Voluntários</option></select></label>
    ...
    <p>Busca pelo início do nome, sem diferenciar maiúsculas, minúsculas ou acentos.{branches && ' Os contatos vêm do cadastro de acessos das coordenações estaduais.'}</p>
    {result && <><p>{result.data.length ? 'Adicione as pessoas que devem receber o formulário.' : 'Nenhuma pessoa encontrada nesta busca.'}</p>{result.data.map(v => <div className={styles.recipientRow} key={v.id}><span><strong>{v.fullName}</strong><small>{v.branchLabel ? `${v.branchLabel} · ` : ''}{v.phone || 'Sem telefone'} · {v.email || 'Sem e-mail'}</small></span><button type="button" onClick={() => onAdd(v)}>Adicionar {v.fullName}</button></div>)}{result.hasMore && <button type="button" onClick={() => load(result.cursor)}>Próximas 10 pessoas</button>}</>}
  </section>;
}
...
function CampaignDetail({ initial, run, onClose }) {
  ...
  return <section>
    ...
    <h2>Fila de envios e respostas</h2><p>WhatsApp e e-mail abrem com a mensagem pronta. Confirme o envio manualmente.</p>
    ...
    {loading ? <p role="status">Carregando solicitações…</p> : <>
      ...
      {page.items.map(r => {
        const link = formLink(r.token); const message = campaign.mode === 'general' ? generalWhatsappMessage(campaign.title, campaign.description, link) : whatsappMessage(r.name, campaign.formTitle, link, campaign.whatsappMessage); const whatsapp = whatsappUrl(r.phone, message); const email = gmailComposeUrl(r.email, campaign.title, message);
        return <article className={styles.card} key={r.id}><h3>{r.name}</h3>{r.branchName && <p>Coordenação estadual: {r.branchName}</p>}<p>{r.phone || 'Sem telefone'} · {r.email || 'Sem e-mail'} · <span className={styles.badge}>{r.status}</span></p>
          <div className={styles.toolbar}>
            ...
            {email && r.status === 'PENDENTE' && !campaign.archived ? <a className={styles.buttonLink} href={email} target="_blank" rel="noopener noreferrer">Enviar por e-mail</a> : <button disabled>Enviar por e-mail{!r.email ? ' (sem e-mail)' : ''}</button>}
...
export default function LinkFormsPage() {
  ...
  async function createCampaign() {
    await run(async () => {
      ...
      const parsed = mode === 'individual' ? validateRecipients(recipients.split('\n').filter(l => l.trim()).map(line => { const [name, phone = '', email = '', ...extra] = line.split(';'); if (extra.length) throw new Error('Use Nome; telefone; e-mail, um por linha.'); return { name: name.trim(), phone: phone.trim(), email: email.trim() }; })) : [];
      ...
      {campaign ? <CampaignDetail key={campaign.id} initial={campaign} run={run} onClose={() => { setCampaign(null); setRevision(v => v + 1); }} /> : <>
        ...
        {tab === 'builder' ? <>
          ...
          <section className={styles.card}><h2>Gerar campanha</h2><label>Título da campanha<input maxLength={160} value={title} onChange={e => setTitle(e.target.value)} placeholder={definition.title} /></label><label>Descrição da campanha<textarea maxLength={3000} value={description} onChange={e => setDescription(e.target.value)} /></label><label>Mensagem adicional para links individuais (opcional)<textarea maxLength={3000} value={whatsappMessageText} onChange={e => setWhatsappMessageText(e.target.value)} placeholder="Deixe vazio para usar a saudação padrão com o nome do destinatário." /></label><p className={styles.help}>No link geral, o texto copiado usa automaticamente o título e a descrição da campanha.</p><label>Prazo de validade<input type="datetime-local" value={expiresAt} onChange={e => setExpiresAt(e.target.value)} /></label><button onClick={() => setExpiresAt(localDate(definition.validityDays))}>Usar validade padrão do formulário</button><label>Tipo de link<select value={mode} onChange={e => setMode(e.target.value)}><option value="individual">Um link por destinatário</option><option value="general">Um link geral compartilhável</option></select></label>
            {mode === 'general' && <label className={styles.check}><input type="checkbox" checked={collectBranch} onChange={e => setCollectBranch(e.target.checked)} />Pedir nome e coordenação estadual ao abrir o link geral</label>}
            {mode === 'individual' ? <><label>Destinatários — um por linha: Nome; telefone (opcional); e-mail (opcional)<textarea rows={6} maxLength={33000} value={recipients} onChange={e => setRecipients(e.target.value)} placeholder={'Maria; 11999999999; maria@exemplo.org\nJoão; ; joao@exemplo.org'} /></label><p>{recipients.split('\n').filter(l => l.trim()).length}/{LIMITS.recipients} destinatários</p></> : <p>Compartilhe o mesmo endereço. Cada participante informa o nome e recebe uma solicitação própria. Limite: {LIMITS.recipients} participantes.</p>}
          </section>
          {mode === 'individual' && <RecipientImport run={run} onAdd={v => { const lines = recipients.split('\n').filter(l => l.trim()); if (lines.length >= LIMITS.recipients) { setError(`Limite de ${LIMITS.recipients} destinatários.`); return; } const line = `${v.fullName.replaceAll(';', ',')}; ${v.phone || ''}; ${v.email || ''}`; if (!lines.includes(line)) setRecipients([...lines, line].join('\n')); }} />}
