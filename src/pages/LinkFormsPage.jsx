  ...
  return <section>
    ...
    {loading ? <p role="status">Carregando solicitações…</p> : <>
      ...
      {page.items.map(r => {
        ...
        return <article className={styles.card} key={r.id}><h3>{r.name}</h3>{r.branchName && <p>Coordenação estadual: {r.branchName}</p>}<p>{r.phone || 'Sem telefone'} · <span className={styles.badge}>{r.status}</span></p>
