...
import { createSuperAdmin, deleteSuperAdmin, listSuperAdmins } from "../services/branchViewersService";
...
export default function SuperAdminsPage() {
  ...
  return <main className={styles.page}><section className={styles.card}><p className={styles.eyebrow}>Acesso restrito</p><h1>Novo superAdmin</h1><p>Somente um superAdmin ativo pode criar outro administrador nacional.</p><form onSubmit={submit}><label>Nome completo<input required minLength="2" value={form.displayName} onChange={(e) => setForm({ ...form, displayName: e.target.value })} /></label><label>E-mail de acesso<ContactInput required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label>Telefone para compartilhar por WhatsApp <small>(opcional e não será salvo)</small><ContactInput type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label>{error && <p role="alert" className={styles.error}>{error}</p>}<button disabled={busy}>{busy ? "Criando..." : "Criar superAdmin"}</button></form>{credentials && <aside className={styles.credentials}><strong>Acesso criado</strong><span>E-mail: {credentials.email}</span><span>Senha temporária: <b>{credentials.password}</b></span><div className={styles.shareActions}>{shareUrl && <a className={styles.whatsapp} href={shareUrl} target="_blank" rel="noreferrer">Enviar por WhatsApp</a>}{emailUrl && <a className={styles.email} href={emailUrl} target="_blank" rel="noreferrer">Enviar por e-mail</a>}</div><small>Copie a senha agora. Ela não será exibida novamente.</small></aside>}</section><section className={styles.listCard} aria-labelledby="superadmins-title"><div><p className={styles.eyebrow}>Acessos ativos</p><h2 id="superadmins-title">Superadmins cadastrados</h2></div>{loadingAdmins ? <p aria-busy="true">Carregando...</p> : listError ? <p className={styles.error}>{listError} <button type="button" onClick={loadAdmins}>Tentar novamente</button></p> : admins.length === 0 ? <p>Nenhum superadmin cadastrado.</p> : <ul>{admins.map((admin) => <li key={admin.id}><strong>{admin.displayName}</strong><span>{admin.email}</span><small>{admin.status === "active" ? "Ativo" : "Bloqueado"}</small>{admin.id !== user?.uid && <button className={styles.deleteButton} type="button" disabled={busy} onClick={() => removeAdmin(admin)}>Excluir superadmin</button>}</li>)}</ul>}</section></main>;
}
